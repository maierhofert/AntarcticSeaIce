...
all_data = readRDS("../data/complete_dataset/all_dataPC.RDS")

all_data = readRDS("../data/complete_dataset/all_dataLag.RDS")
names(all_data)[-(1:9)] = paste0("V", names(all_data)[-(1:9)])

# subset data to after 1979
all_dat = all_data[all_data$Date >= as.Date("1979-01-01"),]
# names(all_dat)[-(1:9)] = paste0("V", names(all_dat)[-(1:9)])
all_dat$total = as.vector(all_dat$total)
all_dat$King_Hakon = as.vector(all_dat$King_Hakon)
all_dat$Ross = as.vector(all_dat$Ross)
all_dat$East_Antarctica = as.vector(all_dat$East_Antarctica)
all_dat$Weddell = as.vector(all_dat$Weddell)
all_dat$Bellingshausen_Amundsen_Sea = as.vector(all_dat$Bellingshausen_Amundsen_Sea)

# set up a nice cross validation
# sectors
sector = colnames(all_dat)[c(4:8,3)]
pretty_sector = c("King Hakon VII", "Ross Sea", "East Antarctica", "Weddell Sea", 
                  "Bellingshausen\nAmundsen Sea", "Total")
# blocks for cross validation
year_block = list(1979:1983, 1984:1988,
                  1989:1993, 1994:1998,
                  1999:2003, 2004:2008,
                  2009:2013, 2014:2018)
...
mod_list = list()
# TODO include total as sum of sectors
for (i in 1:nrow(res_data)) {
  ...
  #                         paste0("bbs(Month, by = ",
  #                                covariates, #[1:100],
  #                                ", knots = 4, boundary.knots = c(0.5, 12.5), cyclic = TRUE)"), collapse = " + ")) %>%
  #   formula()
  this.formula = paste0(this.sector,
                        " ~ ",
                        paste(
  ...
  # sort(mboost::varimp(lm_ab_annualcycle))

    
  # # # chuck it into random forest instead
  # this.formula = paste0(this.sector,
  #                       " ~ Month + ",
  #                       paste(covariates, #[1:155],
  #                             collapse = " + ")) %>%
  #   formula()
  ...
  res_data$mse0[i] = mse(0, all_dat[test_id, this.sector])
  res_data$mse[i] = mse(preds, all_dat[test_id, this.sector])
...
res_data %>%
  group_by(sector) %>% 
  summarize(rmse0 = sqrt(mean(mse0)),
            rmse = sqrt(mean(mse))) %>%
  mutate(rmse0 = round(rmse0, digits = 2),
         rmse = round(rmse, digits = 2))
# seasonally boosted model with mstop = 100 (mstop = 300, 500 performs slightly worse, but mostly the same, 1000 performs definitely worse)
# # A tibble: 6 × 3
# sector                      rmse0  rmse
# <fct>                       <dbl> <dbl>
# 1 King_Hakon                   0.29  0.28
# 2 Ross                         0.35  0.32
# 3 East_Antarctica              0.18  0.16
# 4 Weddell                      0.31  0.25
# 5 Bellingshausen_Amundsen_Sea  0.17  0.14
# 6 total                        0.55  0.53
# seasonally boosted model based on 50 PC with mstop = 300 (40PC and mstop=200 performs worse, 100PC mstop=300 performs similarly well)
# A tibble: 6 × 3
# sector                      rmse0  rmse
# <fct>                       <dbl> <dbl>
# 1 King_Hakon                   0.29  0.28
# 2 Ross                         0.35  0.3 
# 3 East_Antarctica              0.18  0.18
# 4 Weddell                      0.31  0.27
# 5 Bellingshausen_Amundsen_Sea  0.17  0.16
# 6 total                        0.55  0.53

# not seasonal linear model with mstop = 200 (mstop = 100 is nbasically the same)
# A tibble: 6 × 3
# sector                      rmse0  rmse
# <fct>                       <dbl> <dbl>
# 1 King_Hakon                   0.29  0.26
# 2 Ross                         0.35  0.3
# 3 East_Antarctica              0.18  0.16
# 4 Weddell                      0.31  0.24
# 5 Bellingshausen_Amundsen_Sea  0.17  0.15
# 6 total                        0.55  0.54
