...
# "/home/handcock/Grants/NSF-OPP-1745089/REC_MSH/code"
...
load("../data/all_dat_monthly_imputed.RData")
all_dat = readRDS("../../../../../thomasm/RECONSTRUCTION/data/complete_dataset/all_dataPC.RDS")
# subset to first 100PC
...
buff <- all_dat$tdate >= 1979 & all_dat$tdate < 2021.6
...
#all_pc <- prcomp(all_x, center=TRUE, scale=TRUE)$x[,1:300]
dim(all_pc)
dim(all_x)

Kar <- 4
Kma <- 0
# how does the specification of priors work?
mdl <- arima(sie,order = c(Kar,0,Kma),xreg = all_x, 
             slab_scale=0.025, tprior=FALSE,
             series.name = "all",seldat=no1978,
             lagAR=c(1,2,3,12),lagMA=0, p_nonzero=p_nonzero)
# # regression model without time series components
#mdl <- reg(sie,xreg = all_x #slab_scale=0.025,
#           series.name = "all",seldat=no1978, p_nonzero=p_nonzero)

if(T){
  fit <- varstan(mdl, iter=1000, chains=chains, tree.depth=10)
  saveRDS(fit, file = paste0("bc.",fname,".od.",vname,".rds"))
}else{
  fit <- readRDS(file = paste0("bc.",fname,".od.",vname,".rds"))
}
pdf(paste0("f.",fname,".od.",vname,".pdf"))
check_residuals(fit)
...
c01_beta <- summary(fit, pars=colnames(fit$model$X))
...
samp[[1]][1:20]
#samp <- as.matrix(extract(fit$stanfit, pars = "breg"))
#samp[[1]][1:20]
...
pdf(paste0("g.",fname,".od.",vname,".pdf"))
plot(loo1)
print(loo1)
}
