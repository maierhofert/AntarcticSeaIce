...
# all_dat and all_x differ by a bufer
...
# set to -1000 becuase that's what stan checks
all_dat$sie[is.na(all_dat$sie)] <- -1000
# use matrix for multivariate data
...
Kma <- 0
# d=1 is hard coded
# ts can be multivariate, fix that in bayesbackcast
# 
mdl <- arima(sie,order = c(Kar,1,Kma),xreg = all_pc, #slab_scale=0.25, 
             series.name = "all",seldat=no1978,lagAR=c(1,2,3,12),lagMA=0, 
             p_nonzero=p_nonzero, tprior = TRUE)
# mdl <- reg(sie,xreg = all_pc #slab_scale=0.025,
#           series.name = "all",seldat=no1978, p_nonzero=p_nonzero)

if(T){
  fit <- varstan(mdl, iter=2000, chains=chains, tree.depth=15, tprior=TRUE)
...
c01_beta <- summary(fit) #  summary(fit, pars=colnames(fit$model$X))
