...
all_data = readRDS("../data/complete_dataset/all_data_seasonal_lagged.RDS")

# subset data to after 1979
all_dat = all_data[all_data$season_year >= 1979,]
...
for (i in 1:nrow(res_data)) {
  ...
  test_id = between(all_dat$season_year, 
                    first_year,
                    last_year)
  train_id = !test_id
  ...
  this.formula = paste0(this.sector,
                        " ~ ", # season is included in covariates
